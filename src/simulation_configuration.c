    ...
    if (strlen(buffer) > 0)
    {
      ...
      if (hasValue(buffer))
      {
        ...
        if (strstr(buffer, "DT") != NULL)
          simulation_configuration->dt = getIntValue(buffer);
        if (strstr(buffer, "DISPLAY_PROGRESS_FREQUENCY") != NULL)
          simulation_configuration->display_progess_frequency = getIntValue(buffer);
        if (strstr(buffer, "WRITE_REACTOR_STATE_FREQUENCY") != NULL)
          simulation_configuration->write_reactor_state_frequency = getIntValue(buffer);
        if (strstr(buffer, "SIZE_X") != NULL)
          simulation_configuration->size_x = getIntValue(buffer);
        if (strstr(buffer, "SIZE_Y") != NULL)
          simulation_configuration->size_y = getIntValue(buffer);
        if (strstr(buffer, "SIZE_Z") != NULL)
          simulation_configuration->size_z = getIntValue(buffer);
        if (strstr(buffer, "MOD_WEIGHT") != NULL)
          simulation_configuration->moderator_weight = getIntValue(buffer);
        if (strstr(buffer, "COLLISION_PROB_MULTIPLYER") != NULL)
...
{
  if (strcmp(name, "U235") == 0)
    return U235_CONFIG;
  if (strcmp(name, "U238") == 0)
    return U238_CONFIG;
  if (strcmp(name, "Pu239") == 0)
    return Pu239_CONFIG;
  if (strcmp(name, "U236") == 0)
    return U236_CONFIG;
  if (strcmp(name, "Ba141") == 0)
    return Ba141_CONFIG;
  if (strcmp(name, "Kr92") == 0)
    return Kr92_CONFIG;
  if (strcmp(name, "Xe140") == 0)
    return Xe140_CONFIG;
  if (strcmp(name, "Sr94") == 0)
    return Sr94_CONFIG;
  if (strcmp(name, "Xe134") == 0)
    return Xe134_CONFIG;
  if (strcmp(name, "Zr103") == 0)
    return Zr103_CONFIG;
  if (strcmp(name, "Pu240") == 0)
    return Pu240_CONFIG;
